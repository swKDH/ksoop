        ...
        private void change_Click_1(object sender, EventArgs e)
        {
            try
            {
                using (MySqlConnection mysql = new MySqlConnection(_connectionAddress))
                {
                    ...
                    string updateQuery = 
                   string.Format("UPDATE survey SET name = '{1}', sex = '{2}', rockchic = '{3}', minimal = '{4}', cityboy = '{5}'  WHERE id={0};", index, textBox1.Text, textBox2.Text, textBox3.Text, textBox4.Text, textBox5.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                using (MySqlConnection mysql = new MySqlConnection(_connectionAddress))
                {
                    ...
                    MySqlCommand command = new MySqlCommand(selectQuery, mysql);
                    MySqlDataReader table = command.ExecuteReader();//메소드 호출 후 확인을 위한 메소드
